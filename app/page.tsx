...
export default function Home() {
  ...
  async function getCard(type : string){
    ...
    const responseInfo = await fetch(`http://pokebackend-durj.onrender.com/pokemonInfo?type="${type}"`);
    const card = await responseInfo.json();

    setStatus("Generating Image (~15 seconds)");
    const imageResponse = await fetch(`http://pokebackend-durj.onrender.com/pokemonImage?prompt=${card["imageGen"]}`);
